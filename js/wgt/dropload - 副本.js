...
define("wgt/dropload",function(require, exports, module) {
	return function(obj){
		var page=1,$wrap=$('.aui-list'),$load=$('.dropload-down'),id=sessionStorage.tid||0;
		$(obj).dropload({
		    scrollArea : window,
		    //autoLoad:false,
		    loadDownFn : function(me){
		    	//'./js/wgt/test.json?page='
		    	page++;
		    	$.getJSON(U.cate+'?game_from=2&type='+id+'&page='+page+'&callback=?',function(result){
		    		if(result.data.length!=0){
		    			$load.show();
						var liHtml=tpl("cateChild",{catCls:'cate-btn1',padCls:'',resp:result.data.gamelist});
						//console.log(liHtml);
						$wrap.append(liHtml);
						me.resetload();
		    		}else{
